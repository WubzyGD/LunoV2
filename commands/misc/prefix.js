...
module.exports = {
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (!args.length) {return message.channel.send(`Syntax: \`${prefix} <newPrefix|clear>\`. My current prefix in this server is \`${tguild.prefix.length ? tguild.prefix : 'l.'}\``);}
        let np = args[0];
        ...
        if (['c', 'clear', 'n', 'none'].includes(np.trim().toLowerCase())) {
            ...
            return message.reply('this server\'s prefix has been reset to the default, `l.`.');
        }
